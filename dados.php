<?php

$produtos = [   
    [        

        "foto" => "imgs/geladeira.jpg",

        "nome" => "Geladeira Brastemp Frost Free",
 
        "valor" => "De R$ 3.800,00 ",

        "valordesconto" => "Por apenas  R$ 3.399,99", 
        
        "descricao" => "
            Marca: Brastemp;
            <br>
            Modelo: BRE80AK;
            <br>
            Cor: Inox;
            <br>
            Tipo de painel de controle: Externo;
            <br>
            Capacidade: 394 L;
            <br>
            Capacidade do freezer: 179 L;
            <br>
            Peso: 108 kg;
            <br>
            Quantidade de níveis de temperatura: 5;
            <br>
            Eficiência energética: A;
            <br>
            Tipo de degelo: Frost free;
        ",      

        "garantia" => "Garantia: 12 meses." 
   
    ],
    
    [        

        "foto" => "imgs/maquina_lavar.jpg",

        "nome" => "Máquina de lavar automática Brastemp ",

        "valor" => " De R$ 2.900,00",

        "valordesconto" => "Por apenas R$ 2.299,99", 
         "descricao" => "
            Marca: Brastemp;
            <br>
            Modelo: BWK16AB;
            <br>
            Cor: Prata;
            <br>
            Eficiência energética: A;
            <br>
            Capacidade: 16 kg;
            <br>
            Peso: 45 kg;
            <br>
            Quantidade de programas de lavagem: 12;
            <br>
            Níveis de água: 4;
            <br>
            Sistema de lavagem: Americano;
            <br>
            Tipos de carga de água: Manual, Automática;
        ",

        "garantia" => "Garantia: 16 meses." 
   

    ],

    [        

        "foto" => "imgs/micro_ondas.jpg",

        "nome" => "Microondas Panasonic",

        "valor" => "De R$ 1.000,00",
        
        "valordesconto" => "Por apenas R$ 799,99",
        
        "descricao" => "
        Modelo: ST66NBRU;
        <br>
        Cor: Preto;
        <br>
        Potência (W): 900W;
        <br>
        Capacidade: 34L;
        <br>
        Níveis de potência: 10;
        <br>
        Peso (kg): 14,8;
        <br>
        Altura (cm): 32,5;
        <br>
        Largura (cm): 52,0;
        <br>
        Comprimento (cm): 43,2;
        <br>
        SKU: 220V: 4968523 | 127V: 4968560;
        ",

        "garantia" => "Garantia: 8 meses." 
    ],

    [        
        "foto" => "imgs/televisao.jpg",

        "nome" => "Smart Tv Tcl Led 32 Polegadas",

        "valor" => "De R$ 2.200,00",

        "valordesconto" => "Por apenas R$ 1.799,99",
    ],

    [        
        "foto" => "imgs/motocerra - Copia.jpg",

        "nome" => "Motosserra a gasolina Toyama Farmer 49.2cc",

        "valor" => "De R$ 900,00",

        "valordesconto" => "Por apenas R$ 749,99",
    ],

    [      
        "foto" => "imgs/quadro.jpg",

        "nome" => "Quadro Decorativo 129x63",

        "valor" => "De R$ 200,00",

        "valordesconto" => "Por apenas R$ 159,99",
    ],

    [        
        "foto" => "imgs/notbook.webp",

        "nome" => "Dell Inspiron 5000 I15-5566-A50B",

        "valor" => "De R$ 3.900,00",

        "valordesconto" => "Por apenas R$ 3.399,99",

    ],

    [        
        "foto" => "imgs/maquina_cabelo.jpg",

        "nome" => "Kit Aparador De Cabelo E Pelos",

        "valor" => "De R$190,00",

        "valordesconto" => "Por apenas R$ 159,99",
    ],

];
